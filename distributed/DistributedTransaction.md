...
![](../assets/164d65a2768cd3d6.png)
我们的本地事务由资源管理器进行管理:
![](../assets/164d65b4f31b2356.png)
而事务的ACID是通过InnoDB日志和锁来保证。<u>Mysql事务的原子性和隔离性是通过数据库锁的机制实现的，持久性通过redo log（重做日志）来实现，一致性和Rollback通过Undo log来实现</u>。UndoLog的原理很简单，为了满足事务的原子性，在操作任何数据之前，首先将数据备份到一个地方（这个存储数据备份的地方称为UndoLog）。然后进行数据的修改。如果出现了错误或者用户执行了ROLLBACK语句，系统可以利用Undo Log中的备份将数据恢复到事务开始之前的状态。
